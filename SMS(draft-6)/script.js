...
function logMood() {
  ...
  entries.push({ mood: currentMood, note, ts: Date.now(), hour: new Date().getHours(), intensity: currentIntensity });
  ...
  if (isDifficult && sc >= 3) {
    ...
    const intLabel = ['','lightly','','intensely'][currentIntensity];
    html = `<div class="alert alert-calm">
      <div class="alert-title">Logged ✓</div>
      <div style="font-size:13px; margin-top:3px;">
        ${POSITIVE_MOODS.includes(currentMood) ? 'Great to hear — feeling ' + intLabel + ' ' + moodInfo[currentMood].label.toLowerCase() + '! 🌟' : 'Thanks for checking in today 💜'}
      </div>
    </div>`;
  }
...
function renderHistory() {
  ...
  el.innerHTML = [...entries].reverse().slice(0, 8).map(e => {
    ...
    const intBadge = e.intensity ? `<span class="int-badge int-${e.intensity}">${intensityLabels[e.intensity]}</span>` : '';
    return `<div class="hrow">
      <div class="hemoji">${m.emoji}</div>
      <div class="hinfo">
        <div class="hlabel">${m.label}${intBadge}<span class="badge ${m.badge}">${m.cat}</span></div>
...
function openCalDay(y, m, d) {
  ...
  if (!dayEntries.length) {
    ...
    body.innerHTML = dayEntries.map(e => {
      ...
      const intLabel = e.intensity ? intensityLabels[e.intensity] : '';
      return `<div class="modal-entry">
        <div class="modal-entry-top">
          <span class="modal-emoji">${mi.emoji}</span>
          <div>
            <div class="modal-mood-name">${mi.label}${intLabel ? ` · <span class="modal-intensity">${intLabel} intensity</span>` : ''}</div>
