...
const HOSPITALS = {
  bengaluru: [
    { name:'NIMHANS',          type:'Mental Health Institute', dist:'3.2 km', open:true,  icon:'🧠', maps:'https://maps.google.com/?q=NIMHANS+Bengaluru' },
    { name:'Manipal Hospital', type:'Multi-specialty',         dist:'5.1 km', open:true,  icon:'🏥', maps:'https://maps.google.com/?q=Manipal+Hospital+Bengaluru' },
    { name:'Apollo Hospital',  type:'Multi-specialty',         dist:'8.2 km', open:false, icon:'🏥', maps:'https://maps.google.com/?q=Apollo+Hospital+Bengaluru' }
  ],
  mumbai: [
    { name:'KEM Hospital',  type:'Government Hospital', dist:'4.1 km', open:true, icon:'🏥', maps:'https://maps.google.com/?q=KEM+Hospital+Mumbai' },
    { name:'iCall Clinic',  type:'Mental Health',       dist:'5.5 km', open:true, icon:'🧠', maps:'https://maps.google.com/?q=iCall+Mumbai' }
  ],
  delhi: [
    { name:'AIIMS Psychiatry', type:'Mental Health Institute', dist:'6.2 km', open:true, icon:'🧠', maps:'https://maps.google.com/?q=AIIMS+Delhi' },
    { name:'Max Hospital',     type:'Multi-specialty',         dist:'8.1 km', open:true, icon:'🏥', maps:'https://maps.google.com/?q=Max+Hospital+Delhi' }
  ],
  chennai: [
    { name:'Institute of Mental Health', type:'Mental Health',  dist:'4.8 km', open:true, icon:'🧠', maps:'https://maps.google.com/?q=Institute+of+Mental+Health+Chennai' },
    { name:'Apollo Hospital',            type:'Multi-specialty', dist:'6.2 km', open:true, icon:'🏥', maps:'https://maps.google.com/?q=Apollo+Hospital+Chennai' }
  ],
  default: [
    { name:'Government Hospital', type:'General',       dist:'Nearby', open:true, icon:'🏥', maps:'https://maps.google.com/?q=government+hospital+near+me' },
    { name:'Mental Health Clinic', type:'Mental Health', dist:'Nearby', open:true, icon:'🧠', maps:'https://maps.google.com/?q=mental+health+clinic+near+me' }
  ]
};
...
let xp            = parseInt(localStorage.getItem('ssms7_xp')      || '0');
...
function resetAll() {
  if (confirm('Reset all data? This cannot be undone.')) {
    ['ssms7_e','ssms7_p','ssms7_xp','ssms7_focus'].forEach(k => localStorage.removeItem(k));
    location.reload();
  }
}

// ─── XP — silent, bar only, no popup ──────────────────────────────────────────────────
function addXP(amount) {
  xp += amount;
  localStorage.setItem('ssms7_xp', xp);
  updateXPBar();
}
...
function showTab(id, el) {
  ...
  if (id === 'calendar') { renderCalendar(); renderReport(); }
    ...
    <li>
      ...
      <div style="flex:1">${s}
        <div class="sugg-confirm" id="sc-${Date.now()}-${i}" onclick="completeSugg(this)">
          <input type="checkbox"> Mark as done (+5 XP)
        </div>
      </div>
    </li>`).join('');
...
function completeSugg(el) {
  ...
  if (!cb.checked) { cb.checked = true; el.classList.add('done'); addXP(5); }
}
  ...
  const tick = () => {
    ...
    if (rem > 0) {
      ...
      if (breathePhaseIdx >= p.phases.length) {
        breathePhaseIdx = 0;
        breatheCycles++;
        if (breatheCycles === 1) addXP(15); // silent XP for first cycle
      }
  ...
  el.innerHTML = ts.length
    ? ts.map(s => `
        <div class="session-row">
          ...
          <span class="session-xp">${s.completed ? '+20 XP' : ''}</span>
        </div>`).join('')
...
function searchHospitals() {
  const city = document.getElementById('city-inp').value.trim().toLowerCase();
  const list = HOSPITALS[city] || HOSPITALS.default;
  document.getElementById('hospital-list').innerHTML = list.map(h => `
    <div class="hosp-card">
      <div class="hosp-icon">${h.icon}</div>
      <div style="flex:1;">
        <div class="hosp-name">${h.name}</div>
        <div class="hosp-type">${h.type}</div>
        <div class="hosp-meta">
          <span class="hosp-dist">📍 ${h.dist}</span>
          <span class="status-pill ${h.open ? 'pill-open' : 'pill-closed'}">${h.open ? 'Open' : 'Closed'}</span>
        </div>
        <a href="${h.maps}" target="_blank">
          <button class="dir-btn" style="margin-top:8px;">📍 Get Directions</button>
        </a>
      </div>
    </div>`).join('');
}
